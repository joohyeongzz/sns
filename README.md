...
너무 많은 SADD 커맨드를 요청하면 Redis의 Thread가 밀려 CPU 사용량이 증가할 수 있는 문제가 잠재적으로 남아있습니다.
마찬가지로 로컬 캐시에 좋아요 데이터를 저장했다가, 주기적으로 스케줄러를 활용하여 Redis에 Flush하는 방법을 고려할 수 있습니다.
로컬 캐시 동기화를 위한 시스템 복잡도를 고려하여, 일단은 보류하기로 하였습니다.

# Redis 기반 데이터 캐시
읽기 작업이 자주 발생하는 게시글 데이터 특성 상 병목 현상을 줄이기 위해 Redis를 캐시 저장소로 사용하였습니다.
게시글 데이터는 Look Aside 방식을 기본 읽기 전략으로 선택하되, 수정이 빈번하지 않기에 Write Though 방식을 통해 데이터 정합성을 지켰습니다.
