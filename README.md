...
- ### Resilience4j 활용 재시도 로직 및 서킷 브레이커 구현 및 생성 실패 피드 복구 작업
...
파이프라이닝을 실행하는 메서드는 @Order 어노테이션을 사용하여 재시도 AOP가 우선 작동하게 설정하였습니다.

재시도가 최종 실패하거나 Circuit이 Open 상태일 때 실행된 피드 생성 작업은 복구 작업 큐에 추가됩니다.
큐에 과도한 작업이 쌓여 메모리 사용량이 증가할 수 있는 문제를 방지하기 위해 큐 크기를 제한하였습니다.
Circuit이 Close 상태이고 서버에 PING을 주기적으로 보내면서 정상적으로 응답한다면 복구 작업을 진행합니다.
각 메서드가 하나의 파이프라인 단위를 가지기 떄문에, 별도의 배치 사이즈를 설정하지 않고 메서드 자체를 복구 작업의 단위로 정하였습니다.
복구 작업 시에 실행되는 메서드에도 @RedisPipeline 어노테이션을 설정하여 중복되는 코드를 최소화하였습니다.

![스크린샷 2024-11-30 221127](https://github.com/user-attachments/assets/a472e8a6-5195-4db3-92c8-6d3696bd3a73)
![스크린샷 2024-11-30 221420](https://github.com/user-attachments/assets/49ba480b-5560-49e3-9042-4d792523dd86)
![스크린샷 2024-11-30 221141](https://github.com/user-attachments/assets/e1093b70-40fd-43f7-8892-093f5b5097cd)
![스크린샷 2024-11-30 221605](https://github.com/user-attachments/assets/453d2b58-128e-4972-803f-b76557feb750)
![스크린샷 2024-11-30 221556](https://github.com/user-attachments/assets/318cd7d7-9205-41bd-a098-8decbd647e32)
![스크린샷 2024-11-30 222013](https://github.com/user-attachments/assets/9be92d0c-5a10-4abb-9f29-80a7cc255402)
...
- ### Redis 기반 데이터 캐시
웹 서버는 RESTful API를 통해 클라이언트의 요청을 처리하며, 대부분의 요청이 데이터베이스 작업을 수반합니다.
이 과정에서 DB와의 네트워크 통신 지연과 디스크 I/O로 인해 스레드가 장시간 BLOCKED 상태에 머무르게 되어,
대규모 트래픽 상황에서는 데이터베이스 작업이 주요 병목 지점이 될 수 있습니다.

이러한 문제를 해결하기 위해 먼저 Redis와 같은 인메모리 데이터베이스를 도입하여 빈번한 조회 데이터를 캐싱함으로써 
디스크 I/O를 메모리 접근으로 대체하여 응답 속도를 개선했습니다.
